...
def solution(image, current_speed, current_steering):
    ...
    radar = cv2.inRange(hsv, lower_red, upper_red)
    GaripBiSey2= cv2.moments(radar)
    h,w,_ = hsv.shape
    if GaripBiSey2['m00'] > 0:
        tehlike = int(GaripBiSey2['m10'] / GaripBiSey2['m00'])
        steering = (tehlike + 25 - w/2 )* 0.02
        speed = 5
